...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_slash_w_is_a_shortcut_for_a_word_character_class
    ...
    assert_equal "variable_1", "variable_1 = 42"[/[a-zA-Z0-9_]+/]   # whitespaces
    assert_equal "variable_1", "variable_1 = 42"[/\w+/]             # equivalent
  end
  ...
  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/]        # left-most rule
    assert_equal nil, "start end"[/start\z/]
  end

  def test_caret_anchors_to_the_start_of_lines
    assert_equal "2", "num 42\n2 lines"[/^\d+/]       # left-most rule...
  end
  ...
  def test_parentheses_group_contents
    assert_equal "hahaha", "ahahaha"[/(ha)+/]             # [] is for options
  end                                                     # () is set of all

  # ------------------------------------------------------------------

  def test_parentheses_also_capture_matched_content_by_number
    assert_equal "Gray", "Gray, James"[/(\w+), (\w+)/, 1]       # here number defines the             
    assert_equal "James", "Gray, James"[/(\w+), (\w+)/, 2]      # second capture (\w+)
  end
